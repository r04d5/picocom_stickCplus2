...
Para tornar esta ferramenta ainda mais poderosa para desenvolvimento de sistemas embarcados e testes de segurança de hardware, as seguintes fases estão planejadas:

*   **Fase 1: Menu Multi-Modos & Visualizador Hex/Binário** (Concluído)
    *   Adiciona um menu de seleção de modo (duplo clique no Botão B).
    *   Adiciona um terminal de `hexdump` ao vivo para visualizar caracteres não imprimíveis.
*   **Fase 2: Detecção Automática de Baudrate** (Pendente)
    *   Medição do tempo de borda dos pulsos de RX para calcular e configurar o baudrate correto de forma automática.
*   **Fase 3: Enviador de Macros & Interruptor de Bootloader** (Pendente)
    *   Envio de pacotes de comandos pré-definidos (Modbus, comandos AT).
    *   Spammer automático de teclado para interromper o bootloader (ex: U-Boot) no momento em que o alvo é ligado.
*   **Fase 4: Ponte Wi-Fi (Túnel Sniffer)** (Pendente)
    *   Encaminhamento da interface UART física para um socket sem fio TCP/WebSocket.
